      ...
      <div class="card_item">
        <div style="margin-left: 10px">
          ...
          <div class="item-small-row">
            温度:{{ g.temperature }}℃
            <span v-if="g.temperature<=25&&g.temperature>=15" class="box-normal">温度正常</span>
            <span v-else-if="g.temperature<15" class="box-danger">温度过低</span>
            <span v-else class="box-danger">温度过高</span>
          </div>
          <div class="item-small-row">
            湿度:{{ g.humidity }}%
            <span v-if="g.humidity<=70&&g.humidity>=50" class="box-normal">湿度正常</span>
            <span v-else-if="g.humidity<50" class="box-danger">环境过于干燥</span>
            <span v-else class="box-danger">环境过于潮湿</span>
          </div>
          <div class="item-small-row">
            CO2浓度:{{ g.co2 }}ppm
            <span v-if="g.co2<=1200&&g.co2>=800" class="box-normal">正常</span>
            <span v-else-if="g.co2<800" class="box-danger">当前浓度过低</span>
          ...
          <div class="item-small-row">
            ...
            <span v-if="g.water_used<=1200&&g.water_used>=800" class="box-normal">正常</span>
            <span v-else-if="g.water_used<800" class="box-danger">缺水</span>
            <span v-else class="box-danger">过多</span>
          </div>
          <div class="item-small-row">
            <el-button type="success" @click="startFertilizerDialog(g)">投放肥料</el-button>
            <span style="margin-left: 10px">当前已投放:{{ g.fertilizer_used }}L</span>
            <span v-if="g.fertilizer_used<=1200&&g.fertilizer_used>=800" class="box-normal">刚好</span>
            <span v-else-if="g.fertilizer_used<800" class="box-danger">缺肥</span>
            <span v-else class="box-danger">过多</span>
          </div>
          <div class="item-small-row">
            ...
            <span v-if="g.pesticide_used<=1200&&g.pesticide_used>=800" class="box-normal">刚好</span>
            <span v-else-if="g.pesticide_used<800" class="box-danger">不足</span>
            <span v-else class="box-danger">过多</span>
          </div>
        </div>
